...
Учебник существует в двух языковых изданиях (`book/ru/` и `book/en/`), описывающих один и тот же движок:
...
bash book/build/build-pdf.sh all   # оба (по умолчанию)
```

Зависимости: `pandoc` 3.x и `xelatex` (TeX Live). Оба издания держатся в синхроне с кодом — pre-commit хук блокирует изменение `packages/`, не затронувшее обе книги (см. [CONTRIBUTING.md](CONTRIBUTING.md)).
